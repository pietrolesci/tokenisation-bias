...
# from https://github.com/cloneofsimo/zeroshampoo/tree/main
class ZeroShampooWithAdamGraftingOptimizer(Optimizer):
    def __init__(
        self,
        params: list | tuple,
        lr: float = 0.001,
    ...
    @torch.no_grad()
    def _make_lookup_and_enumeratables(self) -> None:
        self.lookup = {}
        self.enumeratables = []
        global_counter = 0
        total_params = 0
                    ...
                    for i1 in range(0, s1, self.block_size):
                        ...
                        for i2 in range(0, s2, self.block_size):
                            ...
                            self.enumeratables.append(
                                (global_counter, block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group)
                            )
    ...
    def _enumerate_sharded_params(self):
        for global_counter, block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group in self.enumeratables:
            if global_counter % self.world_size != self.rank:
                continue
            yield block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group

    def _init_state(self) -> None:
        for block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group in self._enumerate_sharded_params():
            block_param = param.view(s1, s2)[i1:i1r, i2:i2r]
            print(
                f"Rank {self.rank} is managing parameter {block_name}, shape: {block_param.shape}, "
                f"dtype: {block_param.dtype}, range {i1}:{i1r}, {i2}:{i2r}"
            )
    ...
    def step(self, *args, **kwargs) -> None:
        ...
        for block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group in self._enumerate_sharded_params():
            ...
            assert block_param.shape == block_grad.shape, (block_param.shape, block_grad.shape)
    ...
    def _check_momentum_and_variance(self):
        ...
        # for block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group in self._enumerate_sharded_params():
        for block_name, _, (_, _), (i1, i1r), (i2, i2r), _ in self._enumerate_sharded_params():
            state = self.state[block_name]
    ...
    def build_global_state_for_debug_purposes(self):
        self.global_state = {}
        # for global_counter, block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group in self.enumeratables:
        for global_counter, block_name, param, (s1, s2), (i1, i1r), (i2, i2r), _ in self.enumeratables:
            if global_counter % self.world_size != self.rank:
                continue
            ...
            if "exp_avg" not in self.global_state[param]:
                self.global_state[param]["exp_avg"] = torch.ones_like(param.data).view(s1, s2)
            if "exp_avg_sq" not in self.global_state[param]:
                self.global_state[param]["exp_avg_sq"] = torch.ones_like(param.data).view(s1, s2)

            print(f"Doing {block_name}, {i1}:{i1r}, {i2}:{i2r}")
            assert self.state[block_name]["m_adam"].shape == (i1r - i1, i2r - i2)
            # fill in
            self.global_state[param]["exp_avg"][i1:i1r, i2:i2r] = self.state[block_name]["m_adam"]
            self.global_state[param]["exp_avg_sq"][i1:i1r, i2:i2r] = self.state[block_name]["v_adam"]
    ...
    def _sync_params(self):
        ...
        # for global_counter, block_name, param, (s1, s2), (i1, i1r), (i2, i2r), group in self.enumeratables:
        for global_counter, _, param, (_, _), (_, _), (_, _), _ in self.enumeratables:
